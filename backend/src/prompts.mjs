...
Combine related criteria into no more than six scorecard dimensions. Focus on no more than six findings and six claims with the greatest decision impact. Every high-severity finding must cite a proposal location or uploaded source. Return only the requested nested structured assessment.`;

export const FAST_DECISION_PROMPT = `${EVIDENCE_RULES}
Act as a skeptical reviewer panel and final adjudicator in one pass. Use exactly three relevant personas selected from program, subject matter, monitoring and evaluation, finance, implementation, risk and compliance, community, technology, and executive reviewers.
...
Name the strongest reason to fund and strongest reason to reject. Limit the failure pre-mortem to three causes and revision priorities to the five changes most likely to affect the decision. Audit citations and flag every material conclusion lacking a proposal reference, source URL, explicit inference label, or [SOURCE NEEDED].
